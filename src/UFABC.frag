...
vec3 getNormal(in vec3 p) {	
    ...
    normal.x = (sdf(p + h.xyy) - sdf(p)) / hOffset;
	normal.y = (sdf(p + h.yxy) - sdf(p)) / hOffset;
	normal.z = (sdf(p + h.yyx) - sdf(p)) / hOffset;
 ...
 */
float rayMarching(vec3 direction){
    float count = 0.0;
    float t = 0.0;
    while(t < D) {
