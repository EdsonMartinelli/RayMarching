 ...
 *
 * UFABC logo in the center of scene created using two sphere and smooth minimum modeling.
 *
...
/**
 * @brief Plane SDF with two sphere used to cut. 
 *
 * A SDF function that use two sphere and smooth minimum to cut the logotype.
 *
 * @param [in] p Normalized 2D pixel position.
 * @return The correct value of SDF at the position.
 */
float sdPlane(vec2 p){
    vec2 offset1 = vec2(1.2, -0.7);
    vec2 offset2 = vec2(0.95, -0.12);
