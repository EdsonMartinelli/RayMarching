 ...
 *
 * Get normal of a given point in the world using a numerical differentiation (Forward Difference) with 
 * 4 sample points. The small value of the method is applied in the three axes (x, y, z).
 *
 * @param [in] p Normalized 3D space position.
...
vec3 getNormal(in vec3 p, float pointValue) {	
    ...
    normal.x = (sdf(p + h.xyy).value - pointValue);
	normal.y = (sdf(p + h.yxy).value - pointValue);
	normal.z = (sdf(p + h.yyx).value - pointValue);
